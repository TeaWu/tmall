...
<div class="sidebar">
    ...
    <div class="widget widget-product-search">
        <form class="form-search" action="shopBySearch">
            <label>
                <input type="text" class="search-field" placeholder="搜索商品…" value=""
                       name="keyword" style="width: 75%; margin-right: 10px"/>
            </label>
        ...
        <div class="widget widget-product-categories">
            ...
            <ul class="product-categories">
                <c:forEach items="${categories}" var="item" varStatus="status">
                    <li><a href="shopBy?cid=${item.id}">${item.name}</a>${countList[status.count-1]}<span
                            class="count"></span></li>
                </c:forEach>
            </ul>
        </div>
        ...
        <ul class="product-list-widget">
            <c:forEach items="${sideGoods}" var="item">
                <li>
                    ...
                    <br>
                    <ins class="price<c:if test="${item.disPrice != null}"> delPrice</c:if>">
                        ¥${item.goodsPrice / 100}</ins>
                    <c:if test="${item.disPrice != null}">
                        <ins class="price">¥${item.disPrice / 100}</ins>
                    </c:if>
                </li>
            </c:forEach>
        </ul>
